...
class ReporteController extends Controller
{
    public function filtros()
    {
        ...
        return ['lista_cursos' => $lista_cursos, 'lista_materias' => $lista_materias, 'lista_periodos' => $lista_periodos, 'lista_rangos' => $lista_rangos];
    }
        ...
        if($asignadoc)
        {
            ...
            if(count($lista_matriculas) > 0)
            {
                foreach($lista_matriculas as $matricula)
                {
                    ...
                    $promedio_actitudinal = ($promedio_actitudinal / ($cantidad_actitudinal == 0 ? 1 : $cantidad_actitudinal)) * $asignadoc->porcentaje_actitudinal / 100;
                    $promedio_individual  = ($promedio_individual / ($cantidad_individual == 0 ? 1 : $cantidad_individual)) * $asignadoc->porcentaje_individual / 100;
                    $promedio_grupal      = ($promedio_grupal / ($cantidad_grupal == 0 ? 1 : $cantidad_grupal)) * $asignadoc->porcentaje_grupal / 100;
    ...
    public function notasDescargar($curso_id, $periodo_id)
    {
        ...
        if($matricula)
        {
            ...
            $pdf = PDF::loadView('web.reporte.notas', ['curso' => $curso, 'alumno' => $alumno, 'periodo' => $periodo ,'lista_reporte' => $lista_reporte, 'lista_rangos' => $lista_rangos, 'jefe_seccion' => $jefe_seccion]);
    ...
    public function cursosDescargar($curso_id, $periodo_id)
    {
        ...
        if(count($matriculas))
        {
            ...
            $pdf = PDF::loadView('web.reporte.cursos', ['curso' => $curso, 'alumno' => $alumno, 'periodo' => $periodo , 'materias' => $materias, 'lista_reporte' => $lista_reporte, 'jefe_departamento' => $jefe_departamento, 'supervisor' => $supervisor]);
