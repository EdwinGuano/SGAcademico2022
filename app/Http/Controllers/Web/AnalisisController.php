...
class AnalisisController extends Controller
{
    ...
    public function CalcularPromedioCurso($periodo_id, $curso_id)
    {
        ...
        $promedio = $nota_total / ($cantidad_notas_materias == 0 ? 1 : $cantidad_notas_materias);
    ...
    public function AprobadosDesaprobados($periodo_id, $curso_id)
    {
        ...
        foreach($lista_matriculas as $matricula)
        {
            ...
            $promedio_curso = round($nota_total / ($cantidad_notas_materias == 0 ? 1 : $cantidad_notas_materias));
    ...
    public function PromedioCursoMayorNota($periodo_id, $curso_id)
    {
        ...
        foreach($lista_matriculas as $matricula)
        {
            ...
            $promedio_curso = $nota_total / ($cantidad_notas_materias == 0 ? 1 : $cantidad_notas_materias);
    ...
    public function PromedioCursoRango($periodo_id, $curso_id, $rango_ids)
    {
        ...
        foreach($lista_matriculas as $matricula)
        {
            ...
            $rango_promedio = RangoPromedio::where('nota_minima', '<=', round($promedio_curso))
                                        ->where('nota_maxima', '>=', round($promedio_curso))
                                        ->first();
            ...
            /*if($rango_promedio->nombre == $rango)
            {
                $aux_rango_promedio = new stdClass();

                $aux_rango_promedio->estudiante   = $matricula->alumnos->nom_al .' '. $matricula->alumnos->ape_al;
                $aux_rango_promedio->nota         = round($promedio_curso);
                $aux_rango_promedio->nombre_rango = $rango_promedio->nombre;

                array_push($lista_rangos, $aux_rango_promedio);
            }*/
        }
